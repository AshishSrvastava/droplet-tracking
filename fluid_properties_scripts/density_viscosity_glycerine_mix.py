...
def density_viscosity_glycerine_mix(fraction_glyc, T):
    ...
    viscosity_glycerol = 0.001 * 12100 * math.exp((-1233 + T) * T / (9900 + 70 * T)) # equation 22. Note factor of 0.001 -> converts to Ns/m^2
    viscosity_water = 0.001 * 1.790 * math.exp((-1230 - T) * T / (36100 + 360 * T)) # equation 21. Again, note conversion to Ns/m^2

    a = 0.705 - 0.0017 * T
    b = (4.9 + 0.036 * T) * a ** 2.5
    alpha = 1 - mass_fraction + (a * b * mass_fraction * (1 - mass_fraction)) / (a * mass_fraction + b * (1 - mass_fraction))
    A = math.log(viscosity_water / viscosity_glycerol) # Note this is NATURAL LOG (ln), not base 10.
    viscosity_mix = viscosity_glycerol * math.exp(A * alpha) # Ns/m^2, equation 6

    # Andreas Volk polynomial:
    c = 1.78E-6 * T ** 2 - 1.82E-4 * T + 1.41E-2
    contraction = 1 + (c * math.sin((mass_fraction) ** 1.31 * math.pi) ** 0.81)
